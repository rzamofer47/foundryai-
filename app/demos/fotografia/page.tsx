...
import JulianCrossMarkup from './JulianCrossMarkup'
import { cssString } from './css-string'
import { JULIAN_SYSTEM } from './system-prompt'

const FONT_LINK_ID = 'julian-cross-demo-fonts'
...
export default function FotografiaPage() {
  useEffect(() => {
    ...
    document.body.style.color = '#111110'
    ...
    if (!link) {
      ...
      link.rel = 'stylesheet'
      link.href =
        'https://fonts.googleapis.com/css2?family=Tenor+Sans&family=Jost:ital,wght@0,200;0,300;0,400;1,200;1,300&display=swap'
      document.head.appendChild(link)
    }

    const root = document.getElementById('julian-cross-root')
    if (!root) {
      return () => {
    ...
    const toggleChat = () => {
      ...
      if (chatOpen && !welcomeShown) {
        ...
        setTimeout(
          () =>
            addBot(
              "Hi! I'm Julian's assistant 📷 I can help with packages, pricing and booking. What are you looking for?"
            ),
          300
        )
      }
    ...
    const send = async () => {
      ...
      try {
        ...
        remTyp()
        addBot('Quick issue — reach Julian at (905) 555-0144 📷')
      }
    }
  ...
  return (
    <>
      ...
      <div id="julian-cross-root" className="min-h-screen w-full bg-[#FAFAF8] text-[#111110]">
        <JulianCrossMarkup />
      </div>
    </>
  )
