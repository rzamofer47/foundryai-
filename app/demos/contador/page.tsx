...
import MeridianAccountingMarkup from './MeridianAccountingMarkup'
import { cssString } from './css-string'
import { MERIDIAN_SYSTEM } from './system-prompt'

const FONT_LINK_ID = 'meridian-accounting-demo-fonts'
...
export default function ContadorPage() {
  useEffect(() => {
    ...
    if (!link) {
      ...
      link.rel = 'stylesheet'
      link.href =
        'https://fonts.googleapis.com/css2?family=Lora:ital,wght@0,400;0,500;0,600;1,400;1,500&family=Mulish:wght@300;400;500;600&display=swap'
      document.head.appendChild(link)
    }

    const root = document.getElementById('meridian-root')
    if (!root) {
      return () => {
    ...
    const toggleChat = () => {
      ...
      if (chatOpen && !welcomeShown) {
        ...
        setTimeout(
          () =>
            addBot(
              "Good day. I'm the Meridian Accounting assistant 📊 I can answer questions about our services, pricing and tax deadlines. How can I help you?"
            ),
          300
        )
      }
    ...
    const send = async () => {
      ...
      try {
        ...
        remTyp()
        addBot('I apologize for the inconvenience. Please call us at (905) 555-0122 📊')
      }
    }
  ...
  return (
    <>
      ...
      <div id="meridian-root" className="min-h-screen w-full bg-[#FAFBFC] text-[#1A2535]">
        <MeridianAccountingMarkup />
      </div>
    </>
  )
}
